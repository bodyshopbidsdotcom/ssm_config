...
RSpec.describe SsmConfig do # do, when, it, expect
    ...
    context "when YAML file is not blank" do
      context "with 0 depth" do
        it "returns correctly" do
          expect(SsmConfig.data2).to eq({"snapsheet"=>{"clients"=>2, "transactions"=>5}, "snapsheet-tx"=>{"clients"=>3, "description"=>"test", "transactions"=>1}})
        end
      end
  ...
  context "When testing environments" do
    ...
    context "when only any and not Rails.env is in the file" do
      it "returns any" do
        expect(SsmConfig.any_only).to eq({"days_to_enter_bank_account"=>{"default"=>3, "company1"=>2, "company2"=>4}})
      end
    end
